...
def execute(filters=None):
	...
	data = []
	group_wise_columns=["parent","item_code","item_name", "customer", "customer_group","territory","sales_person", "posting_date","item_code", "item_name","item_group", "sub_category", "description", \
...
def get_columns(group_wise_columns, filters):
	...
	column_map = frappe._dict({
		...
		"sub_category": _("Sub Category") + ":Link/Brand:100",
...
class GrossProfitGenerator(object):
	...
	def load_invoice_items(self):
		...
		self.si_list = frappe.db.sql("""
			select
				...
				`tabSales Invoice Item`.brand as sub_category, `tabSales Invoice Item`.dn_detail,
	...
	def get_bundle_item_row(self, product_bundle, item):
		item_name, description, item_group, sub_category = self.get_bundle_item_details(item.item_code)
	...
	def get_bundle_item_details(self, item_code):
		return frappe.db.get_value(
			'Item',
			item_code,
			['item_name', 'description', 'item_group', 'sub_category']
		)
