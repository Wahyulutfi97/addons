...
def set_gl_entries_by_account(
	...
	if accounts:
		...
		gl_entries = frappe.db.sql("""select posting_date,voucher_type,voucher_no, account, debit, credit, is_opening, fiscal_year, debit_in_account_currency, credit_in_account_currency, account_currency from `tabGL Entry`
		...
		filter_tax = []
		for entry in gl_entries:
			if entry['voucher_type'] in ["Sales Order", "Delivery Note", "Sales Invoice", "Purchase Order", "Purchase Receipt", "Purchase Invoice", "Payment Entry", "Stock Entry"] :
				data_inv = frappe.db.get_value(entry['voucher_type'], entry['voucher_no'], ['jenis_transaksi'])
				if data_inv == filters['jenis_transaksi']:
					filter_tax.append(entry)
					gl_entries_by_account.setdefault(entry.account, []).append(entry)
			else :
				gl_entries_by_account.setdefault(entry.account, []).append(entry)
